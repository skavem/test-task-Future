...
const BookCard = (book: GoogleBook) => {
  const categories = book.volumeInfo.categories
  const category = book.volumeInfo.mainCategory ?? (categories ? categories[0] : null)
  const images = book.volumeInfo.imageLinks

  return (
    <article className='w-56 flex flex-col'>
      <Link to={`book/${book.id}`} className='overflow-hidden flex-1 flex flex-col justify-center m-2 bg-slate-100 p-5 gap-1 drop-shadow-sm'>
