...
const BookList = ({
  ...
  return (
    <div className='flex items-center w-full flex-col gap-3 py-3'>
      <p className='text-xl w-full text-center'>{pluralise(booksFound, 'book')} found</p>
      
      <div className='flex justify-center flex-wrap w-3/4'>
        {books.map(book => <BookCard {...book} key={book.id} />)}
      </div>
      
      {loading ? <LoaderIcon className='w-8 h-8 text-slate-300 animate-spin fill-slate-900' /> : <></>}
      
      <button 
        type='button' 
        className='bg-slate-700 text-white rounded-md p-3'
        onClick={() => getBooks(books.length)}
      >
        Load more
      </button>
    </div>
